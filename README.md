...
[![Terraform CI](https://github.com/Donte7/lab3-splunk-siem/actions/workflows/terraform-ci.yml/badge.svg)](https://github.com/Donte7/lab3-splunk-siem/actions/workflows/terraform-ci.yml)
...
> **Note on the first badge:** this one is live — it pulls its pass/fail state directly from the actual `terraform-ci.yml` workflow runs in this repo, not a static image. If the pipeline breaks, this badge turns red automatically.
 
**Stack:** Splunk Free (Enterprise trial → free tier) · Azure VM (Ubuntu 22.04) · Terraform (IaC) · GitHub Actions (CI/CD) · Windows Server VM (from Lab 1) · SOC Skills
...
Splunk is the most widely deployed commercial SIEM. Building this lab gives you a concrete, demonstrable skill that shows up on job descriptions for nearly every security operations role — and provisioning it with Terraform, guarded by an automated CI pipeline, demonstrates the Infrastructure-as-Code and DevOps discipline that separates a SOC analyst from a Cloud/Security Engineer.
...
DC01 (Windows Server VM, Lab 1)
  Generates Windows Event Logs — Security (4624/4625/4740/4672), System, Application
...
Every push or pull request touching `terraform/**` is automatically checked by GitHub Actions before it's trusted — codified in [`.github/workflows/terraform-ci.yml`](.github/workflows/terraform-ci.yml).
 
**Current pipeline (Tier 1 — no Azure credentials required):**
 
| Check | What it catches | Why it's safe without Azure secrets |
|---|---|---|
| `terraform fmt -check` | Formatting/style inconsistencies | Reads `.tf` files only, no API calls |
| `terraform validate` | HCL syntax errors, broken resource references | Reads `.tf` files + provider schema, no API calls |
| `tfsec` | Known security anti-patterns (open NSG rules, missing encryption, hardcoded secrets) | Static analysis only, no API calls |
...
**[SCREENSHOT PLACEHOLDER: GitHub Actions run showing both jobs passing green]**
...
├── .github/
│   └── workflows/
│       └── terraform-ci.yml      ← CI/CD pipeline: fmt, validate, tfsec
...
This lab is concrete evidence of SIEM experience, Infrastructure-as-Code discipline, and CI/CD automation for interviews. Be ready to talk through:
...
- ⚙️ **Why** you chose Terraform over ClickOps — reproducibility, drift detection, and the ability to tear down/rebuild an identical environment on demand.
- 🔄 **Why** your CI pipeline stops at Tier 1 (fmt/validate/tfsec) rather than auto-applying to real infrastructure — and what Azure OIDC federation would add if you extended it.
- 🛠️ **What** the CI pipeline actually caught during this build, and how you diagnosed and fixed it — a real, verifiable story beats a pipeline that "just worked."
...
- 🔓 **What** you found when the account-lockout alert never fired, and why root-causing it to `LockoutThreshold=0` was a stronger outcome than assuming the SIEM config was broken.